...
DAtesting <- function(phylo, variables, model, level) {
  ...
  meco <- trans_diff$new(dataset = meco, method = 'linda', formula = model, taxa_level = level)
  
  # Return the results of classification as a data frame
  return(meco)
}

DAtestingPlotBar <- function(meco, factors, palette, number_features, title) {
  
  # Filter the dataframe based on the predefined comparison and significance criteria
  df <- as.data.frame(meco$res_diff)
  filtered_df <- df[df$Significance != "",]
  
  # Select factor of interest if complex model
  if("Factors" %in% colnames(filtered_df)){
    filtered_df <- filtered_df <- filtered_df[filtered_df$Factors %in% factors, ]
  }
  
  # Print number of significant features
  print(paste0("Number of significant features : ", nrow(filtered_df)))
  
  # Sort by smallest P.adj
  filtered_df <- filtered_df[order(filtered_df$P.adj), ]  
  ...
  # Add direction
  if("Factors" %in% colnames(filtered_df)){
    filtered_df$Direction <- ifelse(filtered_df$log2FoldChange<0, paste(factors, "Down"), paste(factors, "Up"))
  }
  else{
    filtered_df$Direction <- ifelse(filtered_df$log2FoldChange>0, strsplit(filtered_df$Comparison, " - ")[[1]][1], strsplit(filtered_df$Comparison, " - ")[[1]][2])
    }
...
DAfeaturesCounts <- function(phylo, paramlist, level = 'ASV') {
  ...
  for (i in 1:length(paramlist)) {
    ...
    df[i, ] <- length(which(da$res_diff$Significance!=""))}
...
# Pathway analysis on DE analysis
...
# Pathway analysis on list
ImmunePathwayAnalysisList <- function(gene_list) {
  library(clusterProfiler)
  library(org.Hs.eg.db)
  
  # Convert gene symbols to Entrez IDs
  genes <- bitr(gene_list, fromType='SYMBOL', toType='ENTREZID', OrgDb='org.Hs.eg.db')
  ...
  # Run pathway analysis
  pathway.KEGG <- as.data.frame(enrichKEGG(genes$ENTREZID, pvalueCutoff = 0.05))
  if (nrow(pathway.KEGG) > 0) {
    pathway.KEGG$Description <- paste("", pathway.KEGG$Description)
    pathway.KEGG$Gene_ratio <- sapply(strsplit(pathway.KEGG$GeneRatio, "/"), function(x) as.numeric(x[1]) / as.numeric(x[2]))
  }
  
  # Add gene symbols to the output
  pathway.KEGG$geneSymbol <- sapply(pathway.KEGG$geneID, convert_entrez_to_symbol)
...
# Plot pathway average with age 
PlotTopPathways <- function(host_nasal_long, immune_pathway_age, n_top_pathways = 20) {
  # Merge each gene data
  merged_data <- pivot_longer(as.data.frame(t(host_nasal_long$E)), cols = everything(),
                              names_to = "gene", values_to = "expression") %>%
    mutate(timepoint = rep(host_nasal_long$targets$TimepointSimplified, each = nrow(host_nasal_long$E)))
  # Filter and sort
  immune_pathway_age_filtered <- immune_pathway_age %>%
    filter(Count >= 4) %>%
    arrange(p.adjust) %>%
    mutate(PathwayIndex = row_number())
  # Formatting
  immune_pathway_age_filtered <- immune_pathway_age_filtered %>%
    mutate(across(where(is.factor), as.character))
  # Select the top pathways
  top_pathways <- immune_pathway_age_filtered %>%
    filter(rank(p.adjust) <= n_top_pathways)
  # Loop through the ranks of the top pathways
  plot_data <- list()
  for (rank in 1:n_top_pathways) {
    pathway_info <- top_pathways %>%
      filter(PathwayIndex == rank)
    selected_pathway_genes <- pathway_info %>%
      pull(geneSymbol) %>%
      as.character() %>%
      strsplit("/") %>%
      unlist()
    pathway_name <- unique(pathway_info$Description)
    direction <- unique(pathway_info$Direction)
    full_pathway_name <- paste(pathway_name, "-", direction)
    pathway_data <- merged_data %>%
      filter(gene %in% selected_pathway_genes) %>%
      mutate(pathway = full_pathway_name, direction = direction)
    
    loess_fits <- pathway_data %>%
      group_by(timepoint) %>%
      summarise(avg_predicted = mean(expression, na.rm = TRUE)) %>%
      mutate(pathway = full_pathway_name, direction = direction)
    plot_data[[rank]] <- loess_fits
  }
  # Combine the data for all top pathways
  all_average_loess <- bind_rows(plot_data)
  # Create the plot
  ggplot(all_average_loess, aes(x = timepoint, y = avg_predicted, group = pathway, color = direction)) +
    geom_line(size = 2, alpha = 0.5) +  # Semi-transparent lines
    scale_color_manual(values = c(custom_palette_age[1], custom_palette_age[3])) +
    #geom_text_repel(data = all_average_loess %>% group_by(pathway) %>% filter(timepoint == max(timepoint)),
    #                aes(x = timepoint, y = avg_predicted, label = pathway), hjust = -0.1, size = 4, nudge_x = 0.5) +
    scale_x_continuous(breaks = c(0, 3, 6, 9, 12, 15)) +
    ylim(c(0,6)) +
    xlab("Age (months)") +
    ylab("Average pathway gene expression (normalised)") +
    theme(legend.position = "none", text = element_text(size = 12))
}

# Plot pathway average with age and individual genes
PlotTopPathwayswGenes <- function(host_nasal_long, immune_pathway_age, pathway_indexes, pathway_colors, genes_to_show = NULL) {
  # Merge each gene data
  merged_data <- pivot_longer(as.data.frame(t(host_nasal_long$E)), cols = everything(),
                              names_to = "gene", values_to = "expression") %>%
    mutate(timepoint = rep(host_nasal_long$targets$TimepointSimplified, each = nrow(host_nasal_long$E)))
  
  # Filter and arrange the immune pathways by p-value
  immune_pathway_age_filtered <- immune_pathway_age %>%
    filter(Count >= 4) %>%
    arrange(p.adjust) %>%
    mutate(PathwayIndex = row_number())
  
  # Ensure that columns are properly formatted to avoid issues with Rle or factors
  immune_pathway_age_filtered <- immune_pathway_age_filtered %>%
    mutate(across(where(is.factor), as.character))
  
  # Prepare plot data
  all_gene_data <- list()  # To store all data for gene plotting
  all_loess_data <- list()  # To store all data for loess curves
  
  # Loop through each selected pathway index
  for (i in seq_along(pathway_indexes)) {
    selected_pathway_index <- pathway_indexes[i]
    
    # Get the genes and info for the selected pathway
    selected_pathway_info <- immune_pathway_age_filtered %>%
      filter(PathwayIndex == selected_pathway_index)
    
    selected_genes <- selected_pathway_info %>%
      pull(geneSymbol) %>%
      as.character() %>%
      strsplit("/") %>%
      unlist()
    
    pathway_name <- unique(selected_pathway_info$Description)
    direction <- unique(selected_pathway_info$Direction)
    
    # Create a unique pathway name by appending the direction (e.g., "cytokine-cytokine - up/down")
    full_pathway_name <- paste(pathway_name, "-", direction)
    
    # Filter merged data for selected genes
    pathway_data <- merged_data %>%
      filter(gene %in% selected_genes) %>%
      mutate(pathway = full_pathway_name, direction = direction)
    
    # Group by gene and timepoint to get average expression for each gene
    pathway_data_avg <- pathway_data %>%
      group_by(gene, timepoint) %>%
      summarise(avg_expression = mean(expression, na.rm = TRUE), .groups = "drop") %>%
      mutate(pathway = full_pathway_name)  # Add unique pathway name
    
    # Calculate the overall average loess curve for the pathway (averaging across all genes)
    average_loess <- pathway_data_avg %>%
      group_by(timepoint) %>%
      summarise(avg_predicted = mean(avg_expression, na.rm = TRUE), .groups = "drop") %>%
      mutate(pathway = full_pathway_name)  # Add unique pathway name
    
    # Store the data for plotting
    all_gene_data[[i]] <- pathway_data_avg
    all_loess_data[[i]] <- average_loess
  }
  
  # Combine all data for the plot
  all_gene_data_combined <- bind_rows(all_gene_data)
  all_loess_data_combined <- bind_rows(all_loess_data)
  
  # Filter the genes to show, if provided
  if (!is.null(genes_to_show)) {
    all_gene_data_combined <- all_gene_data_combined %>%
      filter(gene %in% genes_to_show)
  }
  
  # Plot individual gene curves (only showing specified genes) and the average pathway curves
  p <- ggplot(all_gene_data_combined, aes(x = timepoint, y = avg_expression, group = gene, color = pathway)) +
    # Add individual lines for the specified genes
    geom_line(size = 1, alpha = 1) +
    # Add the average loess curve for each pathway
    geom_line(data = all_loess_data_combined, aes(x = timepoint, y = avg_predicted, group = pathway, color = pathway),
              size = 5, alpha = 0.5, inherit.aes = FALSE) +
    # Label the specified genes at the last timepoint
    geom_text_repel(data = all_gene_data_combined %>% group_by(gene) %>% filter(timepoint == max(timepoint)),
                    aes(x = timepoint, y = avg_expression, label = gene), 
                    size = 3, hjust = -0.1, nudge_x = 0.5) +
    # Label the pathways at the end of the average loess curve
    # geom_text_repel(data = all_loess_data_combined %>% filter(timepoint == max(timepoint)),
    #                aes(x = timepoint, y = avg_predicted), 
    #               size = 5, fontface = "bold", color = "black", nudge_x = 0.5, inherit.aes = FALSE) +
    # Customize the axes
    scale_x_continuous(breaks = c(0, 3, 6, 9, 12, 15)) +
    xlab("Age (months)") + 
    ylab("") +
    # Apply the pathway colors
    scale_color_manual(values = pathway_colors) +
    # Apply minimal theme
    theme(legend.position = "none", text = element_text(size = 12)) +
    ggtitle(all_gene_data_combined$pathway)
...
MOFAfactorloadings <- function(MOFArun, factor, omics, palette, number_features, title){
  ...
  filtered_df$Feature <- gsub("_Nasal|_Blood", "", filtered_df$Feature)
...
BoxplotBinaryMOFAfactor <- function(MOFArun, factor, parameter, title, palette, paletteboxplot) {
  ...
  df.complete <- df[df$Parameter != 'NA' & df$Parameter != 'None', ]
