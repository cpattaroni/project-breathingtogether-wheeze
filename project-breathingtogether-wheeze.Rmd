...
(bact.long.age.bar <- DAtestingPlotBar(bact.long.age.meco, "T4-1year - T1-1week", c(custom_palette_age[1], custom_palette_age[4]), 15, "Longitudunal cohort : Bacteria DA testing with age"))
```
...
```{r eval=T, warning=F, cache=T}
bact.1Y.nomissing <- ps_drop_incomplete(bact.1Y, all)
bact.DA.covariates <- DAtesting(bact.1Y.nomissing,"~Colds+AbxSinceBorn1YEAR", "ASV")


bact.1Y.covariates <- DAfeaturesCounts(bact.1Y, env, "ASV")
fung.1Y.covariates <- DAfeaturesCounts(fung.1Y, all, "ASV")
bact.1Y.covariates <- rbind(bact.1Y.covariates, DAfeaturesCounts(bact.wheeze.1Y, wheeze, "ASV"))
fung.1Y.covariates <- rbind(fung.1Y.covariates, DAfeaturesCounts(fung.wheeze.1Y, wheeze, "ASV"))
```
...
```{r eval=T, warning=F, cache=T}
host.long.pathway <- PathfindRGOKEGG(DE_Age, FDR_param, 0.05, immune = TRUE, do_downregulated = TRUE)
(host.long.pathway.plot <- PlotPathfindR(host.long.pathway, c(custom_palette_age[1], custom_palette_age[4]), number_pathways = 10, min_genecounts = 3,  "Immune pathway analysis : Host nasal longitudinal cohort"))
```
...
```{r}
host.nasal.disease.pathway <- PathfindRGOKEGG(DE_Group_nasal, FDR_param, 0.05, immune = TRUE, do_downregulated = FALSE) # No down regulated immune gene
(host.disease.pathway.plot <- PlotPathfindR(host.nasal.disease.pathway, c(rep(custom_palette_2groups[2], 2)), number_pathways = 10, min_genecounts = 3, "Immune pathway analysis : Host nasal main cohort"))
```
