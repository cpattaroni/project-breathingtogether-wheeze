...
custom_palette_microbes <- c("#F1D495FF","#D69F3DFF","#825716FF")
custom_palette_omics1 <- c("#E7999DFF","#F1D495FF","#D69F3DFF","#825716FF")
custom_palette_omics2 <- c("#E7999DFF","#C83D43FF","#F1D495FF","#D69F3DFF","#825716FF")
```
...
(virus.plot.detection.long <- ggplot(vir.data %>% dplyr::count(AnyVirus, TimepointFactor) %>% mutate(pct = n / sum(n) * 100),
  ...
  labs(title = "Longitudinal cohort : Viral richness with age", x = "TimepointFactor", y = "Percentage of samples") +
...
(virus.plot.details.long <- ggplot(vir.data.detailed, aes(x = variable, y = value, fill = Virus)) +
  ...
  labs(title = "Longitudinal cohort : Viral composition with age", x = "Group", y = "% of positive samples for a given virus") +
  scale_y_continuous(labels = function(x) paste0(x * 100, "%")) +
...
# Beta diversity
(bact.ordi.long <- PlotOrdinationAge(bact.logCSS.long, "Longitudinal cohort: Bacterial weighted Unifrac PCoA", c(custom_palette_age, custom_palette_2groups)))
(fung.ordi.long <- PlotOrdinationAge(fung.logCSS.long, "Longitudinal cohort: Fungal weighted Unifrac PCoA", custom_palette_age))
...
(bact.long.age.bar <- DAtestingPlotBar(bact.long.age.meco, "T4-1year - T1-1week", c(custom_palette_age[1], custom_palette_age[4]), 20, "Longitudunal cohort : Bacterial DA features with age"))
```
...
(alpha.bact.colds <- BoxplotMetadataParameters(bact.1Y, "Observed", "Colds1YEARSimplified", "1 year cohort : Bacterial richness with the number of colds in the first year",
                                              "Observed (log)", custom_palette_2groups, c("grey50","grey50")))

(alpha.bact.abx <- BoxplotMetadataParameters(bact.1Y, "Observed", "AbxSinceBorn1YEARSimplified", "1 year cohort : Bacterial richness with antibiotics treatments in the first year",
                                              "Observed (log)", custom_palette_2groups, c("grey50","grey50")))

(alpha.bact.daycare <- BoxplotMetadataParameters(bact.1Y, "Observed", "DayCare1YEAR", "1 year cohort : Bacterial richness with daycare attendance",
                                              "Observed (log)", custom_palette_2groups, c("grey50","grey50")))

(alpha.bact.siblings <- BoxplotMetadataParameters(bact.1Y, "Observed", "NbSiblingsSimplified", "1 year cohort : Bacterial richness with siblings number",
                                              "Observed (log)", custom_palette_2groups, c("grey50","grey50")))
...
(alpha.fung.season <- BoxplotMetadataParameters(fung.1Y, "Observed", "Season", "1 year cohort : Fungal diversity with sampling season", "Observed (log)",
                                                custom_palette_2groups, "grey50") +
...
(alpha.vir.daycare <- plotanyViruses(vir.1Y, "DayCare1YEAR", "1 year cohort : Viral richness with daycare attendance"))
(alpha.vir.abx <- plotanyViruses(vir.1Y, "AbxSinceBorn1YEARSimplified", "1 year cohort : Viral richness with antibiotics treatments in the first year"))
(alpha.vir.abxpregnancy <- plotanyViruses(vir.1Y, "AbxCoursesPregnancySimplified", "1 year cohort : Viral richness with antibiotics during pregnancy"))
(alpha.vir.delivery <- plotanyViruses(vir.1Y, "DeliverySimplified", "1 year cohort : Viral richness with delivery mode"))
```
...
# Plot
(covariates.da.bact.plot <- PlotfeaturesCounts(bact.1Y.covariates, custom_palette_omics2[3], "1 year cohort : Bacterial DA features with covariates"))
(covariates.da.fung.plot <- PlotfeaturesCounts(fung.1Y.covariates, custom_palette_omics2[4], "1 year cohort : Fungal DA features with covariates"))
```
...
(bact.da.colds.bar <- DAtestingPlotBar(bact.da.colds, "MoreThan2 - LessThan2", c(rep(custom_palette_microbes[1],2)), 7, "1 year cohort : Bacterial DA features with colds in the first year"))

# Day care variable
bact.da.daycare <- DAtesting(bact.1Y, "DayCare1YEAR", "~DayCare1YEAR", "ASV")
(bact.da.daycare.bar <- DAtestingPlotBar(bact.da.daycare, "Yes - No", c(rep(custom_palette_microbes[1],2)), 19, "1 year cohort : Bacterial DA features with daycare attendance"))

# Siblings variable
bact.da.siblings <- DAtesting(bact.1Y, "NbSiblingsSimplified", "~NbSiblingsSimplified", "ASV")
(bact.da.siblings.bar <- DAtestingPlotBar(bact.da.siblings, "One or more sibling(s) - No siblings", c(rep(custom_palette_microbes[1],2)), 7, "1 year cohort : Bacterial DA features with siblings number"))
...
(fung.da.season.bar <- DAtestingPlotBar(fung.da.season, "3-Summer - 1-Winter", c(rep(custom_palette_microbes[2],2)), 11, "1 year cohort : Fungal DA features with sampling season"))
```
...
(bact.Shannon.1Y <- BoxplotMetadataGroup(bact.1Y, "Shannon", "1 year cohort : Bacterial diversity with wheeze", "Shannon diversity", custom_palette_2groups))
(bact.Observed.1Y <- BoxplotMetadataGroup(bact.1Y, "Observed", "1 year cohort : Bacterial richness with wheeze", "Observed richness (log)", custom_palette_2groups))
(fung.Shannon.1Y <- BoxplotMetadataGroup(fung.1Y, "Shannon", "1 year cohort : Bacterial diversity with wheeze", "Shannon diversity", custom_palette_2groups))
...
# Stacked bar plots of viral detection
(virus.plot.wheeze <- plotanyViruses(vir.1Y, "Group", "1 year cohort : Viral richness with wheeze"))
...
(virus.plot.details.wheeze <- ggplot(vir.data.detailed, aes(x = variable, y = value, fill = Virus)) +
    ...
    labs(title = "1 year cohort : Viral composition with wheeze", x = "Group", y = "% of positive samples for a given virus") +
    scale_y_continuous(labels = function(x) paste0(x * 100, "%")) +
...
```{r eval=T, warning=F, cache=T}
# Beta diversity
(bact.ordi.1y <- PlotOrdinationGroupUnifrac(bact.logCSS.1Y , "1 year cohort : Bacterial weighted Unifrac PCoA", custom_palette_2groups))
(fung.ordi.1y <- PlotOrdinationGroupUnifrac(fung.logCSS.1Y , "1 year cohort : Fungal weighted Unifrac PCoA", custom_palette_2groups))
```
...
(bact.da.disease.adj <- DAtestingPlotBar(bact.da.disease.ASV.adj, "Yes - No", custom_palette_2groups, 7, "1 year cohort : DA bacterial features with wheeze (adjusted for daycare attendance)"))
(bact.da.disease <- DAtestingPlotBar(bact.da.disease.ASV, "Yes - No", custom_palette_2groups, 14, "1 year cohort : DA bacterial features with wheeze"))
...
PlotImmunePathways(immune.pathway.age, palette = c(custom_palette_age[1], custom_palette_age[3]), number_pathways = 20, min_genecounts = 5, title = "Test")
```
...
(covariates.de.nasal.plot <- PlotfeaturesCounts(host.nasal.covariates.counts, custom_palette_omics2[1], "1 year cohort : Host nasal DE features with covariates"))
(covariates.de.blood.plot <- PlotfeaturesCounts(host.blood.covariates.counts, custom_palette_omics2[2], "1 year cohort : Host blood DE features with covariates"))
```
...
host.nasal.dbrdaS <- dbRDAselectionMaximum(host.nasal.1Y, covariates.simplified.all)
host.blood.dbrdaS <- dbRDAselectionMaximum(host.dge.blood, covariates.simplified.all)
host.nasal.dbrdaS
host.blood.dbrdaS
```
...
```{r eval=T, warning=F, cache=T}
# Remove samples with missing covariate
host.nasal.1Y.noNA <- host.nasal.1Y[,which(!is.na(host.nasal.1Y$targets$Colds1YEAR))]
row.names(host.nasal.1Y.noNA) <- make.unique(row.names(host.nasal.1Y.noNA))

# Choose design
design.nasal <- model.matrix(~Group+Gender+DayCare1YEAR+Colds1YEAR, data=host.nasal.1Y.noNA$targets)
colnames(design.nasal)

# Get DE genes with age
DE_Group_nasal <- topTable(eBayes(lmFit(host.nasal.1Y.noNA$E, design.nasal)), coef=2, number="Inf")
...
DE_Group_nasal$ID <- rownames(DE_Group_nasal)
...
PlotImmunePathways(immune.pathway.disease, palette = custom_palette_2groups, number_pathways = 20, min_genecounts = 5, title = "Test")
```
...
gene_vars <- apply(Host.matrix.long, 1, var)
sorted_genes <- order(gene_vars, decreasing = TRUE)
top_genes <- head(sorted_genes, 1000)
Host.matrix.long <- Host.matrix.long[top_genes,]
...
# Plot top factors
(MOFAfactorloadings(MOFArun.long, "Factor1", "Nasal", rep(custom.palette.omics1[2],2), 10, "Factor1 : Host nasal"))
(MOFAfactorloadings(MOFArun.long, "Factor2", "Nasal", rep(custom.palette.omics1[2],2), 10, "Factor2 : Host nasal"))
(MOFAfactorloadings(MOFArun.long, "Factor3", "Bacteria", c(custom_palette_age[4], custom_palette_age[1]), 10, "Factor3 : Bacteria"))
(MOFAfactorloadings(MOFArun.long, "Factor3", "Nasal", c(custom_palette_age[4], custom_palette_age[1]),  10, "Factor3 : Host nasal"))
...
(MOFAfactorloadings(MOFArun, "Factor1", "Nasal", custom_palette_2groups, 15, "Factor1 : Nasal host gene expression"))
(MOFAfactorloadings(MOFArun, "Factor1", "Blood", custom_palette_2groups, 15, "Factor1 : Blood host gene expression"))
(MOFAfactorloadings(MOFArun, "Factor1", "Bacteria",custom_palette_2groups, 15, "Factor1 : Bacteria"))

(MOFAfactorloadings(MOFArun, "Factor2", "Nasal", custom_palette_2groups, 15, "Factor2 : Nasal host gene expression"))
(MOFAfactorloadings(MOFArun, "Factor2", "Blood",custom_palette_2groups, 15, "Factor2 : Blood host gene expression"))
(MOFAfactorloadings(MOFArun, "Factor2", "Bacteria",custom_palette_2groups, 15, "Factor2 : Bacteria"))

(MOFAfactorloadings(MOFArun, "Factor3", "Nasal", custom_palette_2groups, 15, "Factor3 : Nasal host gene expression"))
(MOFAfactorloadings(MOFArun, "Factor3", "Blood",custom_palette_2groups, 15, "Factor3 : Blood host gene expression"))
(MOFAfactorloadings(MOFArun, "Factor3", "Bacteria",custom_palette_2groups, 15, "Factor3 : Bacteria"))
```
