...
(alpha.bact.colds <- BoxplotMetadataParameters(bact.1Y, "Observed", "Colds1YEARSimplified", "1YCo : Nasal bacterial richness with the number of colds in the first year",
...
(alpha.bact.abx <- BoxplotMetadataParameters(bact.1Y, "Observed", "AbxSinceBorn1YEARSimplified", "1YCo : Nasal bacterial richness with antibiotics treatments in the first year",
                                              "Observed (log)", c(scales::alpha(custom_palette_microbes[1], 0.65), custom_palette_microbes[1]), c(scales::alpha(custom_palette_microbes[1], 0.7), custom_palette_microbes[1])))

(alpha.bact.daycare <- BoxplotMetadataParameters(bact.1Y, "Observed", "DayCare1YEAR", "1YCo : Bacterial Nasal richness with daycare attendance",
                                              "Observed (log)", c(scales::alpha(custom_palette_microbes[1], 0.65), custom_palette_microbes[1]), c(scales::alpha(custom_palette_microbes[1], 0.7), custom_palette_microbes[1])))

(alpha.bact.siblings <- BoxplotMetadataParameters(bact.1Y, "Observed", "NbSiblingsSimplified", "1YCo : Nasal bacterial richness with siblings number",
                                              "Observed (log)", c(scales::alpha(custom_palette_microbes[1], 0.65), custom_palette_microbes[1]), c(scales::alpha(custom_palette_microbes[1], 0.7), custom_palette_microbes[1])))
...
(alpha.fung.season <- BoxplotMetadataParameters(fung.1Y, "Observed", "Season", "1YCo : Nasal fungal diversity with sampling season", "Observed (log)",
...
(alpha.vir.daycare <- plotanyViruses(vir.1Y, "DayCare1YEAR", "1YCo : Nasal viral richness with daycare attendance"))
(alpha.vir.abx <- plotanyViruses(vir.1Y, "AbxSinceBorn1YEARSimplified", "1YCo : Nasal viral richness with antibiotics treatments in the first year"))
(alpha.vir.abxpregnancy <- plotanyViruses(vir.1Y, "AbxCoursesPregnancySimplified", "1YCo : Nasal viral richness with antibiotics during pregnancy"))
(alpha.vir.delivery <- plotanyViruses(vir.1Y, "DeliverySimplified", "1YCo : Nasal viral richness with delivery mode"))
```
...
# Plot
(covariates.da.bact.plot <- PlotfeaturesCounts(bact.1Y.covariates, custom_palette_omics2[3], "1YCo : Nasal bacterial DA features with covariates"))
(covariates.da.fung.plot <- PlotfeaturesCounts(fung.1Y.covariates, custom_palette_omics2[4], "1YCo : Nasal fungal DA features with covariates"))
```
...
(bact.da.colds.bar <- DAtestingPlotBar(bact.da.colds, "More than 1 colds in the first year - 1 or less colds in the first year", rep(custom_palette_microbes[1],2), 7, "1YCo : Nasal bacterial DA features with colds in the first year"))

# Day care variable
bact.da.daycare <- DAtesting(bact.1Y, "DayCare1YEAR", "~DayCare1YEAR", "ASV")
(bact.da.daycare.bar <- DAtestingPlotBar(bact.da.daycare, "Yes daycare - No daycare", rep(custom_palette_microbes[1],2), 17, "1YCo : Nasal bacterial DA features with daycare attendance"))

# Siblings variable
bact.da.siblings <- DAtesting(bact.1Y, "NbSiblingsSimplified", "~NbSiblingsSimplified", "ASV")
(bact.da.siblings.bar <- DAtestingPlotBar(bact.da.siblings, "One or more sibling(s) - No siblings", rep(custom_palette_microbes[1],2), 7, "1YCo : Nasal bacterial DA features with siblings number"))
...
(fung.da.season.bar <- DAtestingPlotBar(fung.da.season, "3-Summer - 1-Winter", rep(custom_palette_microbes[2],2), 11, "1YCo : Nasal fungal DA features with sampling season"))
```
...
(bact.Shannon.1Y <- BoxplotMetadataGroup(bact.1Y, "Shannon", "1YCo : Nasal bacterial diversity with wheeze", "Shannon diversity", custom_palette_2groups))
(bact.Observed.1Y <- BoxplotMetadataGroup(bact.1Y, "Observed", "1YCo : Nasal bacterial richness with wheeze", "Observed richness (log)", custom_palette_2groups))
(fung.Shannon.1Y <- BoxplotMetadataGroup(fung.1Y, "Shannon", "1YCo : Nasal fungal diversity with wheeze", "Shannon diversity", custom_palette_2groups))
(fung.Observed.1Y <- BoxplotMetadataGroup(fung.1Y, "Observed", "1YCo : Nasal fungal richness with wheeze", "Observed richness (log)", custom_palette_2groups))
(vir.Observed.1Y <- BoxplotMetadataGroup(vir.1Y, "Observed", "1YCo : Nasal viral richness with wheeze", "Observed richness (number of detected viruses)", custom_palette_2groups))
...
# Stacked bar plots of viral detection
(virus.plot.wheeze <- plotanyViruses(vir.1Y, "Group", "1YCo : Nasal viral richness with wheeze"))
...
(virus.plot.details.wheeze <- ggplot(vir.data.detailed, aes(x = variable, y = value, fill = Virus)) +
    ...
    labs(title = "1YCo : Nasal viral composition with wheeze", x = "Group", y = "% of positive samples for a given virus") +
    scale_y_continuous(labels = function(x) paste0(x * 100, "%")) +
...
```{r eval=T, warning=F, cache=T}
# Beta diversity
(bact.ordi.1y <- PlotOrdinationGroupUnifrac(bact.logCSS.1Y , "1YCo : Nasal bacterial weighted Unifrac PCoA", custom_palette_2groups))
(fung.ordi.1y <- PlotOrdinationGroupUnifrac(fung.logCSS.1Y , "1YCo : Nasal fungal weighted Unifrac PCoA", custom_palette_2groups))
```
...
(bact.da.disease.adj <- DAtestingPlotBar(bact.da.disease.ASV.adj, "Yes - No", custom_palette_2groups, 7, "1YCo : Nasal bacterial DA features with wheeze (adjusted for daycare attendance)"))
(bact.da.disease <- DAtestingPlotBar(bact.da.disease.ASV, "Yes - No", custom_palette_2groups, 14, "1YCo : Nasal bacterial DA features with wheeze"))
...
# Bacterial and fungal diversity and richness for 1 year dataset
(alpha.bact.wheeze.episodes <- BoxplotMetadataParameters(bact.wheeze.1Y, "Observed", "MoreThan2EpisodesWheeze", "1YCo : Nasal bacterial richness in wheezers with number of wheeze episodes",
...
# Plot
(covariates.da.bact.wheeze.plot <- PlotfeaturesCounts(bact.wheeze.1Y.covariates, custom_palette_omics2[3], "1YCo : Nasal bacterial DA features with covariates"))
(covariates.da.fung.wheeze.plot <- PlotfeaturesCounts(fung.wheeze.1Y.covariates, custom_palette_omics2[4], "1YCo : Nasal fungal DA features with covariates"))
...
(bact.da.wheeze.episodes <- DAtestingPlotBar(bact.da.wheeze.episodes, "Yes - No", rep(custom_palette_2groups[2], 2), 6, "1YCo : Nasal bacterial DA features wheeze episodes (adjusted for daycare attendance)"))
...
(fung.da.wheeze.episodes <- DAtestingPlotBar(fung.da.wheeze.episodes, "Yes - No", rep(custom_palette_2groups[2], 2), 1, "1YCo : Nasal fungal DA features wheeze episodes (adjusted for sampling season)"))
...
(fung.da.wheeze.hospital <- DAtestingPlotBar(fung.da.wheeze.hospital, "Yes - No", rep(custom_palette_2groups[2], 2), 1, "1YCo : Nasal fungal DA features wheeze hospital admissions (adjusted for sampling season)"))
```
...
(host.nasal.covariates.plot <- PlotfeaturesCounts(host.nasal.covariates.counts, custom_palette_omics2[1],"1YCo : Nasal host DE features with covariates"))
(host.blood.covariates.plot <- PlotfeaturesCounts(host.blood.covariates.counts, custom_palette_omics2[2],"1YCo : Blood host DE features with covariates"))
...
(plot.PCA.nasal.Disease <- ggplot(data.PCA.nasal, aes(x = PC1, y = PC2)) +
  ...
  xlab(paste0("PC1 ", varexp[1], "%")) + ylab(paste0("PC2 ", varexp[2], "%")) + ggtitle("1YCo : Nasal host PCA") +
...
(plot.PCA.blood.Disease <- ggplot(data.PCA.blood, aes(x = PC1, y = PC2)) +
  ...
  scale_fill_manual(values = adjustcolor(custom_palette_2groups, alpha = 0.5)) +
  xlab(paste0("PC1 ", varexp[1], "%")) + ylab(paste0("PC2 ", varexp[2], "%")) + ggtitle("1YCo : Blood host PCA") +
  theme(axis.text.x=element_blank(), axis.text.y=element_blank(), axis.ticks=element_blank(),
          legend.position = "none", legend.box.background=element_rect(), text=element_text(size=12)))
...
# Plot volcano
(plot.volcano.nasal.Disease <- PlotVolcanoGenes(DE_Group_nasal, "Control", c("grey", custom_palette_2groups), "1YCo : Nasal host DE genes with wheeze (adjusted for sex)"))
...
(immune.pathway.disease.plot <- PlotImmunePathways(immune.pathway.disease, palette = custom_palette_2groups[2], number_pathways = 2, min_genecounts = 2, title = "1YCo : Nasal host immune pathways with wheeze (adjusted for sex)"))
```
...
(plot.data.overview <- plot_data_overview(MOFAobject, colors = custom_palette_omics2, covariate = "Bacteria") +
   theme(legend.position = c(1, 0), legend.box.background=element_rect(), text=element_text(size=12)))
(plot.MOFA.var <- MOFAvarianceexplained(MOFArun, custom_palette_omics2, title = "1YCo : MOFA variance explained"))
...
(plot.factor1.nasal.loadings <- MOFAfactorloadings(MOFArun, "Factor1", "Nasal", custom_palette_2groups, 20, "1YCo : Nasal host Factor1 MOFA loadings"))
(plot.factor1.blood.loadings <- MOFAfactorloadings(MOFArun, "Factor1", "Blood", custom_palette_2groups[2], 10, "1YCo : Blood host Factor1 MOFA loadings"))
(plot.factor1.bact.loadings <- MOFAfactorloadings(MOFArun, "Factor1", "Bacteria", custom_palette_2groups, 10, "1YCo : Nasal bacteria Factor1 MOFA loadings"))
```
...
(nasal.blood.OSM <- CorrelationNasalBlood(host.mae.complete, gene = "OSM", title = "1YCo : Host nasal-blood OSM gene expression (matching samples)"))
(nasal.blood.AQP9 <- CorrelationNasalBlood(host.mae.complete, gene = "AQP9", title = "1YCo : Host nasal-blood AQP9 gene expression (matching samples)"))
(nasal.blood.ACVR1C<- CorrelationNasalBlood(host.mae.complete, gene = "ACVR1C", title = "1YCo : Host nasal-blood ACVR1C gene expression (matching samples)"))

# OSM with age
(nasal.OSM.long <- LongitudinalBoxplots(host.nasal.long, "OSM", "LoCo : OSM gene"))
(nasal.AQP9.long <- LongitudinalBoxplots(host.nasal.long, "AQP9", "LoCo : AQP9 gene"))
(nasal.ACVR1C.long <- LongitudinalBoxplots(host.nasal.long, "ACVR1C", "LoCo : ACVR1C gene"))
```
