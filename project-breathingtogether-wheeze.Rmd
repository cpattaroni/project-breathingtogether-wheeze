...
custom_palette_age <- c("pink", "plum3", "plum4", "cadetblue") # for age-related comparisons
```
...
#saveRDS(ensembl, "Databases/ensembl.rds")
ensembl <- readRDS("../Databases/ensembl.rds")
...
# Save plots
ggsave("../Figures/Individual/Host_PCA_Age.pdf", plot.PCA.Age, width = 20, height = 12, units = "cm", useDingbats = FALSE)
```
...
write.xlsx(DE_Age_immune, "../Outputs/Age_DE_results_immune.xlsx", row.names=FALSE)
```
...
metaS.long <- rbind(metaS.1w, metaS.3m, metaS.3m, metaS.1y)

# Prepare metadata file for longitudinal experiment
rownames(metaS) <- paste0("S", metaS$Subjectnumber)

# Set up MOFA in conda
library(reticulate)
use_condaenv(condaenv = "MOFA2", required = TRUE)
```
...
TrainOptions$drop_factor_threshold <- 0.05
TrainOptions$seed <- 2
...
MOFArun.long <- run_mofa(MOFAobject.long, outfile = "../Outputs/MOFA.long.rds", use_basilisk = FALSE)
```
...
```{r}
# Reload MOFA results
(plot.data.overview.long <- plot_data_overview(MOFAobject.long, colors = c(rep("plum4", 5))) +
    theme(legend.position = c(1, 0), legend.box.background=element_rect(), text=element_text(size=12)))
(plot.mofa.explained.long <- plot_variance_explained(MOFArun.long, max_r2 = 15) + scale_fill_gradient(low = "white", high = "plum4") +
    theme(legend.position = c(1, 0), legend.box.background=element_rect(), text=element_text(size=12)))
(r2f <- as.data.frame(melt(MOFArun.long@cache$variance_explained$r2_per_factor[[1]])))
(r2f.plot.long <- ggplot(r2f, aes(fill=Var2, y=value, x=Var1)) +
    scale_fill_manual(values=scales::alpha(colorRampPalette(brewer.pal(10, "RdBu"))(5), 0.6)) +
    geom_bar(position='stack', stat='identity', col='white', cex=0) +
    theme(legend.position = "bottom", legend.box.background=element_rect(), text=element_text(size=12)) +
    labs(x='MOFA+ factor', y='% of variance explained'))

(plot.factor1 <- BoxplotMetadataMOFAfactor(MOFArun.long, "Factor1", "Group", "Latent Factor1", custom_palette_2groups, custom_palette_2groups))
(plot.factor2 <- BoxplotMetadataMOFAfactor(MOFArun.long, "Factor2", "Group", "Latent Factor2", custom_palette_2groups, custom_palette_2groups))
(plot.factor3 <- BoxplotMetadataMOFAfactor(MOFArun.long, "Factor3", "Group", "Latent Factor3", custom_palette_2groups, custom_palette_2groups))

write.xlsx(get_factors(MOFArun.long)$group1, "Outputs/MOFA_weights_long.xlsx")

plot_top_weights(MOFArun.long, view = "Nasal", factor = 1, nfeatures = 20)
plot_top_weights(MOFArun.long, view = "Blood", factor = 1, nfeatures = 20)
plot_top_weights(MOFArun.long, view = "Bacteria", factor = 1, nfeatures = 20)

plot_top_weights(MOFArun.long, view = "Nasal", factor = 3, nfeatures = 20)
plot_top_weights(MOFArun.long, view = "Blood", factor = 3, nfeatures = 20)
plot_top_weights(MOFArun.long, view = "Bacteria", factor = 3, nfeatures = 20)

# Plot loadings for each factor
(plot.host.loadings.factor1 <- plot_top_weights(MOFArun.long, view = "Nasal", factor = 1, nfeatures = 10) +
    geom_segment(aes_string(xend = "feature_id"), size = 2, yend = 0, col = "grey") +
    geom_point(size = 6, col = c(rep(custom_palette_2groups[2], 10))) +
    scale_x_discrete(labels = function(x) str_wrap(x, width = 30)) +
...
# Prepare host longitudinal immune gene matrix
Host.matrix <- host.nasal$E[rownames(host.nasal$E) %in% ImmuneDB$name,]
colnames(Host.matrix) <- paste0('S', host.nasal$targets$Subjectnumber)
gene_vars <- apply(Host.matrix, 1, var)
sorted_genes <- order(gene_vars, decreasing = TRUE)
top_genes <- head(sorted_genes, 1000)
Host.matrix <- Host.matrix[top_genes,]
rownames(Host.matrix) <- make.unique(rownames(Host.matrix))

# Create list of omics datasets
omicslist <- list(Nasal=as.matrix(Host.Nasal.Immune.Genes.matrix), Blood=as.matrix(Host.Blood.Immune.Genes.matrix), Bacteria=as.matrix(Bact.matrix), Fungi=as.matrix(Fung.matrix), Viruses=as.matrix(Vir.matrix))
...
```{r eval=F, warning=F, cache=T}
# Reload MOFA results
(plot.data.overview <- plot_data_overview(MOFAobject, colors = c(rep("plum4", 5))) +
    theme(legend.position = c(1, 0), legend.box.background=element_rect(), text=element_text(size=12)))
(plot.mofa.explained <- plot_variance_explained(MOFArun, max_r2 = 15) + scale_fill_gradient(low = "white", high = "plum4") +
    theme(legend.position = c(1, 0), legend.box.background=element_rect(), text=element_text(size=12)))
(r2f <- as.data.frame(melt(MOFArun@cache$variance_explained$r2_per_factor[[1]])))
(r2f.plot <- ggplot(r2f, aes(fill=Var2, y=value, x=Var1)) +
    scale_fill_manual(values=scales::alpha(colorRampPalette(brewer.pal(10, "RdBu"))(5), 0.6)) +
    geom_bar(position='stack', stat='identity', col='white', cex=0) +
    theme(legend.position = "bottom", legend.box.background=element_rect(), text=element_text(size=12)) +
    labs(x='MOFA+ factor', y='% of variance explained'))

(plot.factor1 <- BoxplotMetadataMOFAfactor(MOFArun, "Factor1", "Group", "Latent Factor1", custom_palette_2groups, custom_palette_2groups))
(plot.factor2 <- BoxplotMetadataMOFAfactor(MOFArun, "Factor2", "Group", "Latent Factor2", custom_palette_2groups, custom_palette_2groups))
(plot.factor3 <- BoxplotMetadataMOFAfactor(MOFArun, "Factor3", "Group", "Latent Factor3", custom_palette_2groups, custom_palette_2groups))

write.xlsx(get_factors(MOFArun)$group1, "Outputs/MOFA_weights.xlsx")

plot_top_weights(MOFArun, view = "Nasal", factor = 1, nfeatures = 20)
plot_top_weights(MOFArun, view = "Blood", factor = 1, nfeatures = 20)
plot_top_weights(MOFArun, view = "Bacteria", factor = 1, nfeatures = 20)

plot_top_weights(MOFArun, view = "Nasal", factor = 3, nfeatures = 20)
plot_top_weights(MOFArun, view = "Blood", factor = 3, nfeatures = 20)
plot_top_weights(MOFArun, view = "Bacteria", factor = 3, nfeatures = 20)

# Plot loadings for each factor
(plot.host.loadings.factor1 <- plot_top_weights(MOFArun, view = "Nasal", factor = 1, nfeatures = 10) +
    geom_segment(aes_string(xend = "feature_id"), size = 2, yend = 0, col = "grey") +
    geom_point(size = 6, col = c(rep(custom_palette_2groups[2], 10))) +
    scale_x_discrete(labels = function(x) str_wrap(x, width = 30)) +
    theme(axis.text.y = element_text(hjust = 1)))

(plot.host.loadings.factor1 <- plot_top_weights(MOFArun, view = "Blood", factor = 1, nfeatures = 10) +
    geom_segment(aes_string(xend = "feature_id"), size = 2, yend = 0, col = "grey") +
    geom_point(size = 6, col = c(rep(custom_palette_2groups[2], 10))) +
    scale_x_discrete(labels = function(x) str_wrap(x, width = 30)) +
    theme(axis.text.y = element_text(hjust = 1)))

(plot.host.loadings.factor1 <- plot_top_weights(MOFArun, view = "host.nasal", factor = 1, nfeatures = 10) +
    geom_segment(aes_string(xend = "feature_id"), size = 2, yend = 0, col = "grey") +
    geom_point(size = 6, col = c(rep(custom_palette_2groups[2], 10))) +
    scale_x_discrete(labels = function(x) str_wrap(x, width = 30)) +
    theme(axis.text.y = element_text(hjust = 1)))

(plot.bact.loadings.factor1 <- plot_top_weights(MOFArun, view = "Bacteria", factor = 1, nfeatures = 10) +
    geom_segment(aes_string(xend = "feature_id"), size = 2, yend = 0, col = "grey") +
    geom_point(size = 6, col = c(rep(custom_palette_2groups[2], 4), rep(custom_palette_2groups[1], 6))) +  
    scale_x_discrete(labels = function(x) str_wrap(x, width = 30)) +
    theme(axis.text.y = element_text(hjust = 1)))

(plot.host.loadings.factor1 <- plot_top_weights(MOFArun, view = "host", factor = 3, nfeatures = 10) +
    geom_segment(aes_string(xend = "feature_id"), size = 2, yend = 0, col = "grey") +
    geom_point(size = 6, col = "black") +
    scale_x_discrete(labels = function(x) str_wrap(x, width = 30)) +
    theme(axis.text.y = element_text(hjust = 1)))

(plot.bact.loadings.factor1 <- plot_top_weights(MOFArun, view = "bact", factor = 3, nfeatures = 10) +
    geom_segment(aes_string(xend = "feature_id"), size = 2, yend = 0, col = "grey") +
    geom_point(size = 6, col = "black") +  
    scale_x_discrete(labels = function(x) str_wrap(x, width = 30)) +
    theme(axis.text.y = element_text(hjust = 1)))
...
```{r eval=F, warning=F, cache=T}
# Extract top features from host and bact
weights.host <- as.matrix(get_weights(MOFArun, scale=FALSE, abs=FALSE)$host)
weights.host <- abs(as.data.frame(sweep(weights.host, MARGIN=2, STATS=colMaxs(abs(weights.host)),"/")))
weights.host.top10 <- rownames(weights.host[order(abs(weights.host$Factor1),decreasing = TRUE),])[1:10]
weights.bact <- as.matrix(get_weights(MOFArun, scale=FALSE, abs=FALSE)$bact)
weights.bact <- abs(as.data.frame(sweep(weights.bact, MARGIN=2, STATS=colMaxs(abs(weights.bact)),"/")))
weights.bact.top10 <- rownames(weights.bact[order(abs(weights.bact$Factor1),decreasing = TRUE),])[1:10]

# Select patients for which both bact and host data (nasal and blood) are available
table(complete.cases(omicsmae))
table(complete.cases(omicsmae[, , 1:2]))
bact.host.mae <- omicsmae[, complete.cases(omicsmae[, , 1:2]), ]

# Subset blood and nasal host data for matching pairs
#nasal.blood.matching <-
```
