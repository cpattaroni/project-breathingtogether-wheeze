...
# Plot volcano
(plot.volcano.nasal.Disease.adj <- PlotVolcanoGenes(DE_group_nasal_adj, "Control", c("grey", custom_palette_2groups), "1 year cohort : DE genes with wheeze (adjusted for daycare attendance and sex)"))
...
# Plot volcano
(plot.volcano.nasal.Disease <- PlotVolcanoGenes(DE_Group_nasal, "Control", c("grey", custom_palette_2groups), "1 year cohort : DE genes with wheeze (adjusted for daycare attendance and sex)"))
...
(plot.data.overview.long <- plot_data_overview(MOFAobject.long, colors = custom_palette_omics1) +
    theme(legend.position = c(1, 0), legend.box.background=element_rect(), text=element_text(size=12)))
(plot.MOFA.var.long <- MOFAvarianceexplained(MOFArun.long, custom_palette_omics1, title = "MOFA longitudinal cohort - variance explained"))

# Plot top factors
(MOFAfactorloadings(MOFArun.long, "Factor1", "Nasal", rep(custom_palette_omics1[2],2), 10, "Factor1 : Host nasal"))
(MOFAfactorloadings(MOFArun.long, "Factor2", "Nasal", rep(custom_palette_omics1[2],2), 10, "Factor2 : Host nasal"))
...
```{r eval=T, warning=F, cache=T}
# Reload MOFA results
(plot.data.overview <- plot_data_overview(MOFAobject, colors = custom_palette_omics2) +
    theme(legend.position = c(1, 0), legend.box.background=element_rect(), text=element_text(size=12)))
(plot.MOFA.var <- MOFAvarianceexplained(MOFArun, custom_palette_omics2, title = "MOFA 1year cohort - variance explained"))
...
(plot.factor1.nasal.loadings <- MOFAfactorloadings(MOFArun, "Factor1", "Nasal", custom_palette_2groups, 20, "Factor1 : Nasal host gene expression"))
...
(plot.factor1.vir.loadings <- MOFAfactorloadings(MOFArun, "Factor1", "Viruses", custom_palette_2groups, 7, "Factor1 : Bacteria"))
(plot.factor1.fung.loadings <- MOFAfactorloadings(MOFArun, "Factor1", "Fungi", custom_palette_2groups, 7, "Factor1 : Bacteria"))
```
...
(nasal.blood.IL36A <- CorrelationNasalBlood(host.mae.complete, gene = "IL36A", title = "1 year cohort : Host nasal-blood IL36A gene expression (matching samples)"))
