...
custom_palette_omics1 <- c("#E7999DFF","gold2","#D69F3DFF","#825716FF")
custom_palette_omics2 <- c("#E7999DFF","#C83D43FF","gold2","#D69F3DFF","#825716FF")
```
...
# Plot the correlations
(all.cor.factors.wheeze <- ggcorrplot(all.cor, method="circle", p.mat=all.p, insig="blank", colors=c("indianred4", "white", "#5C1E91")) +
...
# Bacterial and fungal diversity and richness for 1 year dataset
(alpha.bact.wheeze.episodes <- BoxplotMetadataParameters(bact.wheeze.1Y, "Observed", "MoreThan2EpisodesWheeze", "MaCo : Nasal bacterial richness in wheezers with number of wheeze episodes",
                                                         "Observed (log)", rep(custom_palette_2groups[2],2), rep(custom_palette_2groups[2],2)))
...
(plot.data.overview.long <- plot_data_overview(MOFAobject.long, colors = custom_palette_omics1) +
    theme(legend.position = c(1, 0), legend.box.background=element_rect(), text=element_text(size=12)))
(plot.MOFA.var.long <- MOFAvarianceexplained(MOFArun.long, custom_palette_omics1, title = "LoCo : MOFA variance explained"))

# Plot top factors
(MOFAfactorloadings(MOFArun.long, "Factor2", "Nasal", rep(custom_palette_omics1[2],2), 10, "LoCo : Nasal host Factor2 MOFA loadings"))
(MOFAfactorloadings(MOFArun.long, "Factor2", "Bacteria", c(custom_palette_age[3], custom_palette_age[1]), 10, "LoCo : Nasal bacteria Factor3 MOFA loadings"))
(MOFAfactorloadings(MOFArun.long, "Factor2", "Fungi", c(custom_palette_age[3], custom_palette_age[1]),  10, "LoCo : Nasal host Factor3 MOFA loadings"))

# Plot factor value with age
(MOFAplotAge(MOFArun.long, "Factor2", custom_palette_age, "LoCo : MOFA Factor3 with age"))
```
...
(plot.factor1.group <- BoxplotBinaryMOFAfactor(MOFArun, "Factor1", "HaemoStatus", "Latent Factor1", custom_palette_age, custom_palette_age))
...
(plot.factor1.nasal.loadings <- MOFAfactorloadings(MOFArun, "Factor1", "Nasal", custom_palette_2groups, ntop, "MaCo : Nasal host Factor1 MOFA loadings"))
...
```{r}
# Set the number of top features to investigate
ntop <- 20
...
rownames(Host.matrix.nasal)[grep("CCR", rownames(Host.matrix.nasal))]

# Set up human monocytes markers
human.monocytes.classical <- c("CD14", "CCR2", "CCR5") # CCR2 not detected
human.monocytes.intermediate <- c("CD14", "FCGR3A", "HLA-DRA") # CD16 is FCGR3A
...
nasal.monocytes.cor <- psych::corr.test(nasal.common.genes, nasal.monocytes.markers, method="spearman", adjust="fdr")
nasal.monocytes.cor <- ggcorrplot(nasal.monocytes.cor$r, method = "circle", p.mat = nasal.monocytes.cor$p.adj, 
           ...
           tl.srt = 90, colors = c("goldenrod3", "white", "darkorchid4")) + 
...
blood.monocytes.cor <- psych::corr.test(blood.common.genes, blood.monocytes.genes, method="spearman", adjust="fdr")
ggcorrplot(blood.monocytes.cor$r, method = "circle", p.mat = blood.monocytes.cor$p.adj, 
           insig = "blank",  outline.color = "black", show.diag = TRUE, 
           tl.srt = 90, colors = c("goldenrod3", "white", "darkorchid4")) + 
...
nasal.host.mae <- MultiAssayExperiment(experiments=list(Nasal=as.matrix(Host.matrix.nasal)), colData=DataFrame(metaS))
blood.host.mae <- MultiAssayExperiment(experiments=list(Blood=as.matrix(Host.matrix.blood)), colData=DataFrame(metaS))

# Subset patients for which both blood and nasal gene expression are available
host.mae <- MultiAssayExperiment(experiments=list(Nasal=as.matrix(Host.matrix.nasal), Blood=as.matrix(Host.matrix.blood)), colData=DataFrame(metaS))
table(complete.cases(host.mae))
host.mae.complete <- host.mae[, complete.cases(host.mae), ]

# Subset patients for which both nasal gene expression and nasal bacteria are available
nasal.host.bact.mae <- MultiAssayExperiment(experiments=list(Nasal=as.matrix(Host.matrix.nasal),  Bacteria=as.matrix(Bact.matrix)), colData=DataFrame(metaS))
table(complete.cases(nasal.host.bact.mae))
nasal.host.bact.mae.complete <- nasal.host.bact.mae[, complete.cases(nasal.host.bact.mae ), ]

# Create a dataframe with matching blood 
(nasal.blood.OSM <- CorrelationNasalBlood(host.mae.complete, gene = common.nasal.blood[1], title = "MaCo : Host nasal-blood gene expression (matching samples)"))
(nasal.blood.IL36A <- CorrelationNasalBlood(host.mae.complete, gene = "IL36A", title = "MaCo : Host nasal-blood IL36A gene expression (matching samples)"))
(nasal.blood.EGFR <- CorrelationNasalBlood(host.mae.complete, gene = "EGFR", title = "MaCo : Host nasal-blood EGFR gene expression (matching samples)"))

# OSM pathway investigation
CorrelationGeneBact(nasal.host.bact.mae.complete, gene = "OSM", bacteria = "Haemophilus_influenzae_19", "Nasal OSM vs Haemophilus_influenzae_19 correlation")
CorrelationGeneBact(nasal.host.bact.mae.complete, gene = "OSM", bacteria = "Moraxella_3", "Nasal OSM vs Moraxella_3 correlation")
CorrelationGeneBact(nasal.host.bact.mae.complete, gene = "OSM", bacteria = "Moraxella_catarrhalis_12", "Nasal OSM vs Moraxella_catarrhalis_12 correlation")
CorrelationGeneGene(blood.host.mae, "Blood", "OSM", "EGFR", "OSM vs EGFR")
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "STAT3", "OSM vs STAT3")
CorrelationGeneGene(blood.host.mae, "Blood", "OSM", "EGFR", "Blood OSM vs EGFR")
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "EGFR", "Nasal OSM vs EGFR")
CorrelationGeneGene(blood.host.mae, "Blood", "OSM", "EGFR", "OSM vs EGFR")
CorrelationGeneGene(nasal.host.mae, "Nasal", "CCL20", "EGFR", "OSM vs EGFR")

# OSM receptors investigation
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "OSMR", "Nasal OSM with its receptor OSMR")
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSMR", "LIFR", "Nasal OSM receptors type I: LIFR vs IL6ST (gp130)")
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSMR", "IL6ST", "Nasal OSM receptors type II: OSMR vs IL6ST (gp130)")

# OSM downstream pathways : JAK/STAT
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "JAK1", "Nasal OSM with JAK/STAT pathway member JAK1") # ns
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "JAK2", "Nasal OSM with JAK/STAT pathway member JAK2") # Positive 0.44 ***
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "JAK3", "Nasal OSM with JAK/STAT pathway member JAK3") # Positive 0.8 ***
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "TYK2", "Nasal OSM with JAK/STAT pathway member TYK2") # Positive 0.38 ***
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "STAT1", "Nasal OSM with JAK/STAT pathway member STAT1") # ns
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "STAT2", "Nasal OSM with JAK/STAT pathway member STAT2") # ns
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "STAT3", "Nasal OSM with JAK/STAT pathway member STAT3") # Negative -0.54 ***
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "STAT4", "Nasal OSM with JAK/STAT pathway member STAT4") # ns
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "STAT5B", "Nasal OSM with JAK/STAT pathway member STAT5B") # Positive 0.41 ***

# OSM downstream pathways : ERK pathways
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "IL17RA", "Nasal OSM with ERK pathway member JAK1") # ns

# OSM downstream pathways : ERK1/ERK2
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "MAPK2", "Nasal OSM with JAK/STAT pathway member JAK1") # ns

# OSM with asthma
CorrelationGeneGene(nasal.host.mae, "Nasal", "OSM", "IL33", "Nasal OSM with ERK pathway member JAK1") # ns
...
#### 2) UMAP of nasal subset

```{r}
# Single cell palette
custom_palette_singlecell <- c(paletteer_dynamic("cartography::sand.pal", 4), paletteer_dynamic("cartography::wine.pal", 3), "lightpink4", "grey")

# Reload 
HCLA_data_nose <- readRDS("../Databases/HLCA_complete_nose_comb.rds")
table(HCLA_data_nose@meta.data$tissue_level_3)
table(HCLA_data_nose@meta.data$disease)
sort(table(HCLA_data_nose@meta.data$cell_type), decreasing = TRUE)

# Check elbow plot
ElbowPlot(HCLA_data_nose, ndims = 30)
...
(allcells.ann2.umap <- DimPlot(HCLA_data_nose, group.by = "ann_level_2.2", label = TRUE, repel = TRUE, label.size = 5, raster = FALSE, cols = custom_palette_singlecell) +
    labs(title = "HCLA : core nasal subset") +
    theme(axis.text.x=element_blank(), axis.text.y=element_blank(), axis.ticks=element_blank(),
          legend.box.background=element_rect(), text=element_text(size=12)))

ggsave("../Figures/Individual/Single_cell_ann2.pdf", allcells.ann2.umap, width=20, height=20, units="cm", useDingbats=FALSE)

# List of genes of interest
common.genes <- c("CSF2RA", "IL36A", "TNFSF13B", "TLR8", "AQP9", "OSM", "ACVR1C")

# Check expression of our genes of interest
(allcells.ann2.dotplot <- DotPlot(HCLA_data_nose, features = common.genes, dot.min = 0.005, group.by = "ann_level_2.2", dot.scale = 20, scale.by = "size", assay = "RNA", 
        cols = scales::alpha(c(custom_palette_singlecell[1], custom_palette_singlecell[6]), 0.75)) +
  labs(title = "HCLA : expression of shared local and systemic genes") +
  theme(legend.box.background=element_rect(), text=element_text(size=12)))

ggsave("../Figures/Individual/Single_cell_ann2_dotplot.pdf", allcells.ann2.dotplot, width=20, height=15, units="cm", useDingbats=FALSE)

# Subset the myeloid cells
HCLA_data_nose_myeloid <- subset(HCLA_data_nose, subset = ann_level_2.2 == "Myeloid")
ElbowPlot(HCLA_data_nose_myeloid, ndims = 30)

# Run downstream analyses
HCLA_data_nose_myeloid  <- ScaleData(HCLA_data_nose_myeloid )
HCLA_data_nose_myeloid  <- RunPCA(HCLA_data_nose_myeloid , npcs = 10)
HCLA_data_nose_myeloid  <- RunUMAP(HCLA_data_nose_myeloid , reduction = "pca", dims = 1:10)
HCLA_data_nose_myeloid  <- FindNeighbors(HCLA_data_nose_myeloid , reduction = "pca", dims = 1:10)
HCLA_data_nose_myeloid  <- FindClusters(HCLA_data_nose_myeloid , resolution = 0.5)

# Perform UMAP on the PCA results
p1 <- DimPlot(HCLA_data_nose_myeloid, group.by = "seurat_clusters", label = TRUE, repel = TRUE, label.size = 5, reduction = "umap")
p2 <- DimPlot(HCLA_data_nose_myeloid, group.by = "ann_level_3", label = TRUE, repel = TRUE, label.size = 5, reduction = "umap")
p3 <- DimPlot(HCLA_data_nose_myeloid, group.by = "ann_level_4", label = TRUE, repel = TRUE, label.size = 5, reduction = "umap")
p4 <- DimPlot(HCLA_data_nose_myeloid, group.by = "ann_level_5", label = TRUE, repel = TRUE, label.size = 5, reduction = "umap")
p5 <- DimPlot(HCLA_data_nose_myeloid, group.by = "ann_finest_level", label = TRUE, repel = TRUE, label.size = 5, reduction = "umap")
p6 <- DimPlot(HCLA_data_nose_myeloid, group.by = "cell_type", label = TRUE, repel = TRUE, label.size = 5, reduction = "umap")

t <- ggarrange(p1, p2, p3, p4, p5, p6, nrow = 2, ncol = 3)
ggsave("../Figures/Individual/Single_cell_clusters.pdf", t, width=80, height=50, units="cm", useDingbats=FALSE)

# Re-annotate clusters
FindAllMarkers(HCLA_data_nose_myeloid)
markers %>%
    group_by(cluster) %>%
    top_n(n = 10, wt = avg_log2FC) -> top10
DoHeatmap(HCLA_data_nose_myeloid, features = top10$gene) + NoLegend()


HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation <- ifelse(HCLA_data_nose_myeloid@meta.data$seurat_clusters==0, "Classical monocytes", NA)
HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation <- ifelse(HCLA_data_nose_myeloid@meta.data$seurat_clusters==1, "", HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation)
HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation <- ifelse(HCLA_data_nose_myeloid@meta.data$seurat_clusters==2, "", HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation)
HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation <- ifelse(HCLA_data_nose_myeloid@meta.data$seurat_clusters==3, "", HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation)
HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation <- ifelse(HCLA_data_nose_myeloid@meta.data$seurat_clusters==4, "Mast cells", HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation)
HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation <- ifelse(HCLA_data_nose_myeloid@meta.data$seurat_clusters==5, "", HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation)
HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation <- ifelse(HCLA_data_nose_myeloid@meta.data$seurat_clusters==6, "", HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation)
HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation <- ifelse(HCLA_data_nose_myeloid@meta.data$seurat_clusters==7, "Plasmacytoid dendritic cell", HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation)
HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation <- ifelse(HCLA_data_nose_myeloid@meta.data$seurat_clusters==8, "", HCLA_data_nose_myeloid@meta.data$seurat_clusters_annotation)
...
# Create violin plots for common genes
DotPlot(HCLA_data_nose_myeloid, features = common.genes, dot.min = 0.05, group.by = "cell_type", dot.scale = 20, scale.by = "size")
ggsave("../Figures/Main/Fig5.pdf", width=20, height=20, units="cm", useDingbats=FALSE)
FeaturePlot(HCLA_data_nose_myeloid, features = common.genes, pt.size = 0.5, cols = scales::alpha(c(custom_palette_singlecell[1], custom_palette_singlecell[6]), 0.75)) 
ggsave("../Figures/Main/Fig6.pdf", width=40, height=20, units="cm", useDingbats=FALSE)
```
...
(plot2 <- ggarrange(alpha.bact.colds, NA, alpha.bact.abx, NA, alpha.bact.daycare, NA, alpha.bact.siblings, 
                    ncol = 7, nrow = 1, align = "hv",
                    widths = c(1, 0.25, 1, 0.25, 1, 0.25, 1),
                    labels = c("b","","c", "", "d", "", "e"),
                    font.label = list(size=label.size)))

(plot3 <- ggarrange(alpha.fung.season, NA, alpha.vir.daycare, NA, alpha.vir.abx, NA, alpha.vir.abxpregnancy, NA, alpha.vir.delivery, 
                    ncol = 9, nrow = 1, align = "hv",
                    widths = c(1.5, 0.25, 1, 0.25, 1, 0.25, 1, 0.25, 1),
                    labels = c("f", "", "g","","h", "", "i", "", "j"),
                    font.label = list(size=label.size)))
...
(plot123 <- ggarrange(plot1, plot23,
                     ncol = 2, nrow = 1, align = "hv",
                     labels = c("a", ""),
                     widths = c(1, 4),
                     font.label = list(size=label.size)))

(plot4 <- ggarrange(host.nasal.covariates.plot, NA, host.blood.covariates.plot, NA, covariates.da.bact.plot, NA, covariates.da.fung.plot, 
                    ncol = 7, nrow = 1, align = "hv",
                    widths = c(1, 0.25, 1, 0.25, 1, 0.25, 1),
                    labels = c("k","","l", "", "m", "", "n"),
                    font.label = list(size=label.size)))

(plot5 <- ggarrange(bact.da.colds.bar, NA, bact.da.daycare.bar, NA, bact.da.siblings.bar, NA, fung.da.season.bar, 
                    ncol = 7, nrow = 1, align = "hv",
                    widths = c(1, 0.25, 1, 0.25, 1, 0.25, 1),
                    labels = c("o","","p", "", "q", "", "r"),
                    font.label = list(size=label.size)))

(Figure3 <- ggarrange(plot123, plot4, NA, plot5,
                      ncol = 1, nrow = 4, align = "hv",
                      heights = c(2, 1, 0.1, 1),
                      font.label = list(size=label.size)))

# Save figure
ggsave("../Figures/Main/Figure3.pdf", Figure3, width=55, height=45, units="cm", useDingbats=FALSE)
```
...
# Create plots
(plot1 <- ggarrange(plot.PCA.nasal.Disease, NA, plot.PCA.blood.Disease, NA, bact.ordi.1y,
                    ncol = 5, nrow = 1, align = "hv",
                    widths = c(1, 0.25, 1,  0.25, 1),
                    labels = c("a","","b", "", "c"),
                    font.label = list(size=label.size)))

(plot2.1 <- ggarrange(bact.Observed.1Y, NA, fung.Observed.1Y,
                    ncol = 3, nrow = 1, align = "hv",
                    widths = c(1, 0.1, 1),
                    labels = c("e", "", "f"),
                    font.label = list(size=label.size)))

(plot2.2 <- ggarrange(virus.plot.wheeze, NA, virus.plot.details.wheeze,
                    ncol = 3, nrow = 1, align = "hv",
                    widths = c(1, 0.1, 1),
                    labels = c("g", "", "h"),
                    font.label = list(size=label.size)))

(plot2 <- ggarrange(fung.ordi.1y, NA, plot2.1, NA, plot2.2,
                    ncol = 5, nrow = 1, align = "hv",
                    widths = c(1, 0.25, 1,  0.25, 1),
                    labels = c("d","","", "", ""),
                    font.label = list(size=label.size)))
  
(plot3 <- ggarrange(plot.volcano.nasal.Disease, NA, bact.da.disease, 
                    ncol = 3, nrow = 1, align = "hv",
                    widths = c(2, 0.25, 1.5),
                    labels = c("h","","i"),
                    font.label = list(size=label.size)))

(plot4 <- ggarrange(bact.da.disease.adj, NA, NA, 
                    ncol = 3, nrow = 1, align = "hv",
                    widths = c(1, 1, 1.5),
                    labels = c("j","k","l"),
                    font.label = list(size=label.size)))

(plot5 <- ggarrange(all.cor.factors.wheeze, NA, alpha.bact.wheeze.episodes, NA, bact.da.wheeze.episodes, 
                    ncol = 5, nrow = 1, align = "hv",
...
Figure4 <- ggarrange(plot1, plot2, plot3, plot4, plot5, 
                      ncol = 1, nrow = 5, align = "hv",
                      heights = c(1.2, 1.2, 1.5, 1.2, 1.2),
                      font.label = list(size=label.size))

# Save figure
ggsave("../Figures/Main/Figure4.pdf", Figure4, width=35, height=40, units="cm", useDingbats=FALSE)
```
...
# Create plots
(plot1 <- ggarrange(plot.data.overview, NA, plot.MOFA.var,
                    ncol = 3, nrow = 1, align = "hv",
                    widths = c(1, 0.25, 1),
                    labels = c("a","","b"),
                    font.label = list(size=label.size)))

(plot2 <- ggarrange(plot.factor1.group, NA, plot.factor1.daycaregroup,
                    ncol = 3, nrow = 1, align = "hv",
                    widths = c(1, 0.25, 2),
                    labels = c("c","","d"),
                    font.label = list(size=label.size)))

(plot3 <- plot.factor1.nasal.loadings)

(plot4 <- ggarrange(plot.factor1.blood.loadings, plot.factor1.bact.loadings,
                    ncol = 1, nrow = 2, align = "hv",
                    widths = c(1, 1),
                    labels = c("f","g"),
                    font.label = list(size=label.size)))

(plot34 <- ggarrange(plot3, NA, plot4,
                    ncol = 3, nrow = 1, align = "hv",
                    widths = c(1, 0.25, 1),
                    labels = c("e","",""),
                    font.label = list(size=label.size)))

(plot5 <- ggarrange(nasal.blood.OSM, NA, nasal.blood.AQP9, NA, nasal.blood.ACVR1C,
                    ncol = 5, nrow = 1, align = "hv",
                    widths = c(1, 0.25, 1, 0.25, 1),
                    labels = c("f","","g", "", "h"),
                    font.label = list(size=label.size)))

Figure5 <- ggarrange(plot1, plot2, plot34, plot5, 
                      ncol = 1, nrow = 4, align = "hv",
                      heights = c(1, 1, 1.5, 1),
                      font.label = list(size=label.size))

# Save figure
ggsave("../Figures/Main/Figure5.pdf", Figure5, width=25, height=45, units="cm", useDingbats=FALSE)
```
