...
pdf("../Figures/Individual/Alluvial_groups.pdf", width = 10, height = 5)
...
# Bacterial and fungal diversity and richness for 1 year dataset
(alpha.bact.wheeze.episodes <- BoxplotMetadataParameters(bact.wheeze.1Y, "Observed", "MoreThan2EpisodesWheeze", "1 year cohort : Nasal bacterial richness in wheezers with number of episodes",
...
# Plot
(covariates.da.bact.wheeze.plot <- PlotfeaturesCounts(bact.wheeze.1Y.covariates, custom_palette_omics2[3], "1 year cohort : Nasal bacterial DA features with covariates"))
(covariates.da.fung.wheeze.plot <- PlotfeaturesCounts(fung.wheeze.1Y.covariates, custom_palette_omics2[4], "1 year cohort : Nasal fungal DA features with covariates"))
...
(bact.da.wheeze.episodes <- DAtestingPlotBar(bact.da.wheeze.episodes, "Yes - No", rep(custom_palette_2groups[2], 2), 6, "1 year cohort : Nasal bacterial DA features wheeze episodes (adjusted for daycare attendance)"))
...
(fung.da.wheeze.episodes <- DAtestingPlotBar(fung.da.wheeze.episodes, "Yes - No", rep(custom_palette_2groups[2], 2), 1, "1 year cohort : Nasal fungal DA features wheeze episodes (adjusted for sampling season)"))
...
(fung.da.wheeze.hospital <- DAtestingPlotBar(fung.da.wheeze.hospital, "Yes - No", rep(custom_palette_2groups[2], 2), 1, "1 year cohort : Nasal fungal DA features wheeze hospital admissions (adjusted for sampling season)"))
```
...
# Venn diagram with common/different genes between 2 models
host.nasal.disease.list <- as.vector(na.omit(DE_Group_nasal$label))
host.nasal.disease.adj.list <- as.vector(na.omit(DE_group_nasal_adj$label))
both.list <- list("DE genes with wheeze (sex adjusted)" = host.nasal.disease.list, "DE genes with wheeze (sex and daycare adjusted)" = host.nasal.disease.adj.list)
(host.nasal.venn <- ggVennDiagram(both.list) +
    theme(legend.position = "none") +
    scale_fill_gradient(low = scales::alpha(custom_palette_2groups[2], 0.5), high = custom_palette_2groups[2]) +
    scale_color_manual(values = rep("white", 2)))

# Heatmap with wheeze + daycare specific genes
(host.nasal.common <- intersect(host.nasal.disease.list, host.nasal.disease.adj.list))
host.nasal.common.matrix <- host.nasal.1Y$E[rownames(host.nasal.1Y$E) %in% host.nasal.common,]
host.nasal.common.matrix <- melt(host.nasal.common.matrix, varnames = c("Gene", "Patient"))
host.nasal.common.matrix$Group <- host.nasal.1Y$targets$Group[match(host.nasal.common.matrix$Patient, colnames(host.nasal.1Y$E))]
(host.nasal.common.plot <- ggplot(host.nasal.common.matrix, aes(x = value, y = Gene, color = Group, fill = Group)) + 
    scale_fill_manual(values = adjustcolor(custom_palette_2groups, alpha = 0.5)) +
    scale_color_manual(values = adjustcolor(custom_palette_2groups, alpha = 1)) +
    geom_density_ridges(scale = 1) + labs(x = "Gene expression value (voom normalised)", title = "1 year cohort : Nasal host DE genes with wheeze (sex and daycare adjusted)") +
    theme(legend.position = c(1, 0), legend.box.background=element_rect(), text=element_text(size=12)))
```
...
(plot.factor1.nasal.loadings <- MOFAfactorloadings(MOFArun, "Factor1", "Nasal", custom_palette_2groups, 20, "1 year cohort : Nasal host Factor1 MOFA loadings"))
(plot.factor1.blood.loadings <- MOFAfactorloadings(MOFArun, "Factor1", "Blood", custom_palette_2groups[2], 10, "1 year cohort : Blood host Factor1 MOFA loadings"))
(plot.factor1.bact.loadings <- MOFAfactorloadings(MOFArun, "Factor1", "Bacteria", custom_palette_2groups, 10, "1 year cohort : Nasal bacteria FACTor1 MOFA loadings"))
```
...
(Figure3 <- ggarrange(plot123, plot4, NA, plot5,
                      ncol = 1, nrow = 4, align = "hv",
                      heights = c(2, 1.5, 0.1, 1),
                      font.label = list(size=label.size)))

# Save figure
ggsave("../Figures/Main/Figure3.pdf", Figure3, width=50, height=45, units="cm", useDingbats=FALSE)
```
...
# Create plots
(plot1 <- ggarrange(plot.PCA.nasal.Disease, NA, plot.PCA.blood.Disease, bact.ordi.1y, NA, fung.ordi.1y, 
                    ncol = 3, nrow = 2, align = "hv",
                    widths = c(1, 0.25, 1,  1, 0.25, 1),
                    labels = c("a","","b", "c", "", "d"),
                    font.label = list(size=label.size)))

(plot2 <- ggarrange(bact.Observed.1Y, NA, fung.Observed.1Y, NA, virus.plot.wheeze, NA, virus.plot.details.wheeze, 
                    ncol = 7, nrow = 1, align = "hv",
                    widths = c(1, 0.25, 1, 0.25, 1, 0.25, 1),
                    labels = c("e","","f", "", "g", "", "h"),
                    font.label = list(size=label.size)))
  
(plot3 <- ggarrange(plot.volcano.nasal.Disease, host.nasal.venn,
                    ncol = 2, nrow = 1, align = "hv",
                    widths = c(1.5, 1),
                    labels = c("i", "j"),
                    font.label = list(size=label.size)))

(plot4 <- ggarrange(host.nasal.common.plot, NA, bact.da.disease.adj, NA, bact.da.disease,
                    ncol = 5, nrow = 1, align = "hv",
                    widths = c(1, 0.25, 1, 0.25, 1),
                    labels = c("l","","m", "", "n"),
                    font.label = list(size=label.size)))

(plot5 <- ggarrange(NA, Haemophilus.survival.plot, 
                    ncol = 2, nrow = 1, align = "hv",
                    widths = c(1, 1),
                    labels = c("o", "p"),
                    font.label = list(size=label.size)))

(plot6 <- ggarrange(all.cor.factors.wheeze, NA, alpha.bact.wheeze.episodes, NA, bact.da.wheeze.episodes, 
                    ncol = 7, nrow = 1, align = "hv",
                    widths = c(2, 0.25, 1, 0.25, 1, 0.25, 1),
                    labels = c("q","","r", "", "s"),
                    font.label = list(size=label.size)))

Figure4 <- ggarrange(plot1, plot2, plot3, plot4, plot5, plot6,
                      ncol = 1, nrow = 6, align = "hv",
                      heights = c(3, 1.5, 2, 1.5, 1.5, 1),
                      font.label = list(size=label.size))

# Save figure
ggsave("../Figures/Main/Figure4.pdf", Figure4, width=35, height=60, units="cm", useDingbats=FALSE)
```
