...
all.cor <- rbind(bact.cor$r, fung.cor$r, vir.cor$r)
rownames(all.cor) <- c("Bacterial Diversity", "Bacterial Richness", "Fungal Diversity", "Fungal Richness", "Viral Richness")
colnames(all.cor) <- c("HospitalAdmissionsAsthmaWheeze1YEAR", "EmergencyVisitsAsthmaWheeze1YEAR", "MoreThan2EpisodesWheeze", "DoctorVisitsAsthmaWheeze1YEAR")
...
(all.cor.factors.wheeze <- ggcorrplot(all.cor, method="circle", p.mat=all.p, insig="blank", colors=c("indianred4", "white", "#5C1E91")) +
    labs(title="1 year cohort : Alpha diversity correlations with wheeze covariates") + scale_y_discrete(labels = str_wrap(colnames(all.cor), width=30)) +
    theme(legend.position = c(1, 0), legend.box.background=element_rect(), axis.text.x=element_text(angle=30), text=element_text(size=12)))
...
# Create plots
(plot1 <- ggarrange(plot.PCA.nasal.Disease, NA, plot.PCA.blood.Disease, NA, bact.ordi.1y, NA, fung.ordi.1y, 
                    ncol = 7, nrow = 1, align = "hv",
                    widths = c(1, 0.25, 1, 0.25, 1, 0.25, 1),
                    labels = c("a","","b", "", "c", "", "d"),
                    font.label = list(size=label.size)))
